...
class FlowModelFullRTI3D(FlowModel):
    ...
    def phi(self, x, y, tx, v, delta):
        ...
        tx = tx.view(bs, self.time_embedding_dim, 1, 1, 1).expand(-1, -1, width, depth, height)
        delta = torch.stack([torch.cos(2**k*torch.pi*delta) for k in range(self.time_embedding_dim)], dim=1)
        delta = delta.view(bs, self.time_embedding_dim, 1, 1, 1).expand(-1, -1, width, depth, height)
